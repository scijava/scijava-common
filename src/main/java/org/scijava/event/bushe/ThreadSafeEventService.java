 ...
 * the <tt>CLEANUP_PERIOD_MS_DEFAULT</tt> (20 minutes) by default. The default
...
public class ThreadSafeEventService implements EventService {
	...
	public boolean subscribe(Class cl, EventSubscriber eh) {
		...
		return subscribe(cl, subscribersByEventClass,
			new WeakReference<EventSubscriber>(eh));
	}
	...
	private <T> List<T> getSubscribersByPattern(String topic,
...
//         }
		...
		VetoTopicEventListener vtl, String topic, Object eventObj)
	{
		if (LOG.isLoggable(Level.DEBUG)) {
			if (event != null) {
				LOG.debug("Vetoing event: class=" + event.getClass() + ", event=" +
					event + ", vetoer:" + vl);
			}
			else {
				LOG.debug("Vetoing event: topic=" + topic + ", eventObj=" + eventObj +
					", vetoer:" + vtl);
			}
		}
	}
	...
	public int getCacheSizeForEventClass(Class eventClass) {
		...
		synchronized (cacheLock) {
			if (rawCacheSizesForEventClass == null || rawCacheSizesForEventClass
				.size() == 0)
			{
			...
			if (size != null) {
				...
				for (int i = 0; i < interfaces.length; i++) {
					...
					Integer interfaceSize = (Integer) cacheSizesForEventClass.get(
						anInterface);
	...
	public int getCacheSizeForTopic(String topic) {
		...
		synchronized (cacheLock) {
			if ((rawCacheSizesForTopic == null || (rawCacheSizesForTopic != null &&
				rawCacheSizesForTopic.size() == 0)) &&
				(rawCacheSizesForPattern == null || (rawCacheSizesForPattern != null &&
					rawCacheSizesForPattern.size() == 0)))
			{
				return getDefaultCacheSizePerClassOrTopic();
			}
	...
	protected void subscribeVetoException(final Object event, final String topic,
		final Object eventObj, Throwable e, StackTraceElement[] callingStack,
	...
	protected void onEventException(final String topic, final Object eventObj,
		Throwable e, StackTraceElement[] callingStack,
		EventTopicSubscriber eventTopicSubscriber)
	{
		...
		handleException("handling event", null, topic, eventObj, e, callingStack,
			str);
	}
		...
		StackTraceElement[] callingStack, EventSubscriber eventSubscriber)
	{
		...
		handleException("handling event topic", event, null, null, e, callingStack,
			str);
	}
	 ...
	 */
	protected void handleException(final String action, final Object event,
		final String topic, final Object eventObj, Throwable e,
		StackTraceElement[] callingStack, String sourceString)
	{
		String eventClassString = (event == null ? "none" : event.getClass()
			.getName());
		String eventString = event + "";
		String contextMsg = "Exception " + action + " event class=" +
			eventClassString + ", event=" + eventString + ", topic=" + topic +
			", eventObj=" + eventObj;
			...
			synchronized (listenerLock) {
				...
				for (Map subscriberMap : allSubscriberMaps) {
					...
					for (Object subscription : subscriptions) {
						...
						for (Iterator iter = subscribers.iterator(); iter.hasNext();) {
							Object subscriber = iter.next();
							Object realSubscriber =
								getRealSubscriberAndCleanStaleSubscriberIfNecessary(iter,
									subscriber);
							if (realSubscriber == null) {
								staleCount++;
							}
	...
	private static class PrioritizedSubscriberComparator implements
		Comparator<Prioritized>
	{
	...
	private class PatternWrapper {
		...
		public boolean equals(Object o) {
			...
			if (pattern != null) {
				if (!pattern.equals(that.pattern)) {// give the JVM a shot for forward
																						// compatibility
					return pattern.pattern() != null && this.pattern.pattern().equals(
